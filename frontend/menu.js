...
const Menu = {
  ...
  renderTable() {
    ...
    tbody.innerHTML = items.map(m => `
      <tr>
        <td class="bold" style="font-family:var(--f-head);color:var(--tx3);font-size:13px;font-weight:600">${m.id}</td>
        <td class="bold">${escHtml(m.name)}</td>
        <td>
          <span class="badge" style="background:var(--gold-dim);color:var(--gold);border:1px solid var(--gold-border)">
            ${escHtml(m.category)}
          </span>
        </td>
        <td class="price-col">${formatCurrency(m.price)}</td>
        <td style="max-width:260px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--tx3)">
    ...
    return `
      <div class="form-row">
        <div class="form-group">
          ...
          <input class="form-control" id="mi-name" placeholder="e.g. Truffle Pizza" value="${escHtml(item.name || '')}">
        </div>
        <div class="form-group">
      ...
      <div class="form-row">
        ...
        <div class="form-group">
          ...
          <input class="form-control" id="mi-desc" placeholder="Culinary details..." value="${escHtml(item.description || '')}">
        </div>
      </div>
    `;
  },
  ...
  openAddModal() {
    ...
    const footer = `
      ...
      <button class="btn btn-primary" onclick="Menu.submitAdd()">Add Menu Item</button>
    `;
    modal.open('Add Culinary Item', body, footer);
  },
  ...
  async deleteItem(id, name) {
    const body   = `<p style="color:var(--tx2);margin:8px 0">Are you sure you want to delete <strong style="color:var(--tx1)">"${name}"</strong> from the menu?</p>`;
    const footer = `
      <button class="btn btn-ghost" onclick="modal.close()">Cancel</button>
