...
const Customers = {
  ...
  renderTable(customers) {
    ...
    if (!customers.length) {
      tbody.innerHTML = `<tr><td colspan="6"><div class="empty-state"><div class="empty-icon">👥</div><div class="empty-text">No guests found</div></div></td></tr>`;
      return;
    }
    tbody.innerHTML = customers.map(c => `
      <tr>
        <td style="font-family:var(--f-head);color:var(--tx3);font-size:13px;font-weight:600">${c.id}</td>
        <td class="bold">
          <div style="display:flex;align-items:center;gap:12px">
            <div style="width:34px;height:34px;border-radius:50%;background:linear-gradient(135deg,var(--gold),#b8860b);display:grid;place-items:center;font-size:13px;font-weight:700;color:#080b12;flex-shrink:0;box-shadow:var(--glow-gold)">
        ...
        <td>
          <span style="font-family:var(--f-head);font-size:14px;font-weight:700;color:var(--velvet)">${c.order_count || 0} visits</span>
        </td>
        <td style="font-family:var(--f-head);font-weight:700;color:var(--emerald)">${formatCurrency(c.total_spent)}</td>
        <td>
  ...
  openAddModal() {
    const footer = `
      ...
      <button class="btn btn-primary" onclick="Customers.submitAdd()">Add Guest Profile</button>
    `;
    modal.open('Add Guest Profile', this.customerForm(), footer);
  },
  ...
  async submitAdd() {
    ...
    if (!name) { toast.show('Guest name is required', 'error'); return; }
    try {
      await api.post('/customers', { name, phone });
      modal.close();
      toast.show(`Guest profile "${name}" created!`, 'success');
  ...
  async submitEdit(id) {
    ...
    try {
      ...
      modal.close();
      toast.show('Guest profile updated!', 'success');
      this.fetch(document.getElementById('customer-search')?.value || '');
    } catch(e) { toast.show('Error: ' + e.message, 'error'); }
  },

  async deleteCustomer(id, name) {
    const body   = `<p style="color:var(--tx2);margin:8px 0">Delete guest profile <strong style="color:var(--tx1)">"${name}"</strong>? Their order telemetry will remain intact.</p>`;
    const footer = `
      <button class="btn btn-ghost" onclick="modal.close()">Cancel</button>
      <button class="btn btn-danger" onclick="Customers.confirmDelete(${id})">Delete Profile</button>
    `;
    modal.open('Delete Guest Profile', body, footer);
  },
  ...
  async confirmDelete(id) {
    try {
      ...
      modal.close();
      toast.show('Guest profile deleted', 'success');
      this.fetch('');
    } catch(e) { toast.show('Error: ' + e.message, 'error'); }
  },
  ...
  async viewHistory(id) {
    try {
      ...
      const orderRows = orders.length
        ? orders.map(o => `
            <tr>
              <td style="font-family:var(--f-head);color:var(--gold);font-weight:700;font-size:13px">#${o.id}</td>
          ...
          `).join('')
        : `<tr><td colspan="5"><div class="empty-state" style="padding:30px"><div class="empty-text">No order history yet</div></div></td></tr>`;
      ...
      const body = `
        <div class="customer-meta">
          ...
          <div class="meta-item">
            ...
            <div class="meta-label">Lifetime Value</div>
          </div>
          <div class="meta-item">
            <div class="meta-val" style="font-size:16px">${escHtml(customer.phone) || '—'}</div>
            <div class="meta-label">Phone Contact</div>
          </div>
        </div>
        <div class="card-title" style="margin-bottom:12px">Dining History</div>
      ...
      `;
      modal.open(`${customer.name} — Guest Portfolio`, body, footer);
    } catch(e) { toast.show('Error loading guest profile: ' + e.message, 'error'); }
  }
};
